...
//        }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        vm.fetchText(completion: { paragraphs, error in
            ...
            DispatchQueue.main.async {
                let font = self.findFontSize(table: table) ?? UIFont.preferredFont(forTextStyle: .body)
                self.content = self.convert(paragraphs: p, font: font)
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        let characterIndices = self.content.flatMap {
                return $0 as? Line
            } .map {
                return $0.firstCharacterIndex
        }
        
        let wordIndices = self.content.flatMap {
            return $0 as? Line
            } .map {
                return $0.firstWordIndex
        }
        
        vm.closeArticle(wordIndicies: wordIndices, characterIndicies: characterIndices, complete: self.complete)
    }
